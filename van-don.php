<?php require_once('block/head.php'); ?>

<body>
    <div class="wrap">
        <!-- Header -->
        <?php require_once('block/header.php'); ?>
        <!-- Content -->
        <div class="content">
            <section class="section-banner">
                <img src="images/img-content.jpg" alt="" />
                <div class="container pd-15-rp">
                    <div class="breadcrumb-banner">
                        <h2 class="banner-title">Vận đơn</h2>
                        <p>
                            <a href="">Trang chủ</a>
                            <i class="fa fa-angle-right"></i>
                            <a href="">Vận đơn</a>
                            <i class="fa fa-angle-right"></i>
                            <a href="">Export</a>
                        </p>
                    </div>
                </div>
            </section>
            <!-- Content -->
            <section class="section-body-news">
                <div class="container">
                    <div class="row">
                        <div class="col-md-4 col-xs-12">
                            <article class="body-left">
                                <div class="section-follow">
                                    <div class="fb-page" data-href="https://www.facebook.com/vivicorp/" data-width="440" data-hide-cover="false" data-show-facepile="true">
                                    </div>
                                    <!-- Video -->
                                    <video id="my-video" class="video-js" controls preload="auto" poster="images/bg-video.jpg" data-setup="{}">
                                        <source src="https://cdn.selz.com/plyr/1.5/View_From_A_Blue_Moon_Trailer-HD.mp4" type='video/mp4'>
                                        <source src="https://cdn.selz.com/plyr/1.5/View_From_A_Blue_Moon_Trailer-HD.webm" type='video/webm'>
                                        <p class="vjs-no-js">
                                            To view this video please enable JavaScript, and consider upgrading to a web browser that
                                            <a href="http://videojs.com/html5-video-support/" target="_blank">supports HTML5 video</a>
                                        </p>
                                    </video>
                                </div>
                            </article>
                        </div>
                        <div class="col-md-8 col-xs-12">
                            <article class="body-right">
                                <div class="form-1">
                                    <h2>Track air Cargo</h2>
                                    <form action="" class="form-als">
                                        <input type="text" name="" value="61899712502">
                                        <select class="" name="">
                                            <option value="">Export</option>
                                            <option value="">Import</option>
                                        </select>
                                        <button type="button" class="btn-als">Tìm kiếm</button>
                                        <div><button type="button" class="btn-als btn-search">Tìm kiếm nâng cao</button></div>
                                    </form>
                                    <p class="export-result">Kết quả Export: </p>
                                </div>
                                <div class="table-responsive">
                                    <div class="table-1">
                                        <p class="name-table">Gerneral information:</p>
                                        <table class="table">
                                            <thead>
                                                <tr>
                                                    <th>AWB:</th>
                                                    <th colspan="5">618- 9971 2502</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <tr>
                                                    <td>HAWB:</td>
                                                    <td></td>
                                                    <td>Flight Number</td>
                                                    <td>SQ175</td>
                                                    <td>Origin</td>
                                                    <td>HAN</td>
                                                </tr>
                                                <tr>
                                                    <td>Pieces:</td>
                                                    <td>8</td>
                                                    <td>Flight Date</td>
                                                    <td>SQ175</td>
                                                    <td>Origin</td>
                                                    <td>HAN</td>
                                                </tr>
                                                <tr>
                                                    <td>Weight:</td>
                                                    <td>1830</td>
                                                    <td>Flight Time</td>
                                                    <td>SQ175</td>
                                                    <td>Origin</td>
                                                    <td>HAN</td>
                                                </tr>
                                                <tr>
                                                    <td>Commodity</td>
                                                    <td>Consol</td>
                                                    <td>Reamark</td>
                                                    <td colspan="3">DOC/BUP THEO FFM/17H/:7H-ALSB,7H-ALSE,3H-ALST</td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                </div>

                                <div class="table-responsive">
                                    <div class="table-2">
                                        <p class="name-table">Cargo status:</p>
                                        <table class="table">
                                            <thead>
                                                <tr>
                                                    <th>No.</th>
                                                    <th>Status</th>
                                                    <th></th>
                                                    <th>Station<br>Warehouse</th>
                                                    <th>Event<br>Time</th>
                                                    <th>Pieces</th>
                                                    <th>Weight</th>
                                                    <th>Remark</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <tr>
                                                    <td>1</td>
                                                    <td>Received from Shipper (RCS)</td>
                                                    <td></td>
                                                    <td>ALSE</td>
                                                    <td>13 Jan 2016<br>10:40</td>
                                                    <td>8</td>
                                                    <td>1830</td>
                                                    <td></td>
                                                </tr>
                                                <tr>
                                                    <td>2</td>
                                                    <td>Received from Shipper (RCS)</td>
                                                    <td></td>
                                                    <td>ALSE</td>
                                                    <td>13 Jan 2016<br>10:40</td>
                                                    <td>8</td>
                                                    <td>1830</td>
                                                    <td></td>
                                                </tr>
                                                <tr>
                                                    <td>3</td>
                                                    <td>Received from Shipper (RCS)</td>
                                                    <td></td>
                                                    <td>ALSE</td>
                                                    <td>13 Jan 2016<br>10:40</td>
                                                    <td>8</td>
                                                    <td>1830</td>
                                                    <td></td>
                                                </tr>
                                                <tr>
                                                    <td>4</td>
                                                    <td>Received from Shipper (RCS)</td>
                                                    <td></td>
                                                    <td>ALSE</td>
                                                    <td>13 Jan 2016<br>10:40</td>
                                                    <td>8</td>
                                                    <td>1830</td>
                                                    <td></td>
                                                </tr>
                                                <tr>
                                                    <td>5</td>
                                                    <td>Received from Shipper (RCS)</td>
                                                    <td></td>
                                                    <td>ALSE</td>
                                                    <td>13 Jan 2016<br>10:40</td>
                                                    <td>8</td>
                                                    <td>1830</td>
                                                    <td></td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                </div>
                                <div class="table-responsive">
                                    <div class="table-2">
                                        <p class="name-table">More detail:</p>
                                        <table class="table">
                                            <thead>
                                                <tr>
                                                    <th>DNN number</th>
                                                    <th>Hawb</th>
                                                    <th>ID Pcs</th>
                                                    <th>Pieces</th>
                                                    <th>Weight</th>
                                                    <th>Dimention</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <tr>
                                                    <td>8005693381</td>
                                                    <td>7GWA358</td>
                                                    <td>1</td>
                                                    <td>1</td>
                                                    <td>207</td>
                                                    <td>120 x 80 x 107</td>
                                                </tr>
                                                <tr>
                                                    <td>8005693381</td>
                                                    <td>7GWA358</td>
                                                    <td>1</td>
                                                    <td>1</td>
                                                    <td>207</td>
                                                    <td>120 x 80 x 107</td>
                                                </tr>
                                                <tr>
                                                    <td>8005693381</td>
                                                    <td>7GWA358</td>
                                                    <td>1</td>
                                                    <td>1</td>
                                                    <td>207</td>
                                                    <td>120 x 80 x 107</td>
                                                </tr>
                                                <tr>
                                                    <td>8005693381</td>
                                                    <td>7GWA358</td>
                                                    <td>1</td>
                                                    <td>1</td>
                                                    <td>207</td>
                                                    <td>120 x 80 x 107</td>
                                                </tr>
                                                <tr>
                                                    <td>8005693381</td>
                                                    <td>7GWA358</td>
                                                    <td>1</td>
                                                    <td>1</td>
                                                    <td>207</td>
                                                    <td>120 x 80 x 107</td>
                                                </tr>
                                                <tr>
                                                    <td>8005693381</td>
                                                    <td>7GWA358</td>
                                                    <td>1</td>
                                                    <td>1</td>
                                                    <td>207</td>
                                                    <td>120 x 80 x 107</td>
                                                </tr>
                                                <tr>
                                                    <td>8005693381</td>
                                                    <td>7GWA358</td>
                                                    <td>1</td>
                                                    <td>1</td>
                                                    <td>207</td>
                                                    <td>120 x 80 x 107</td>
                                                </tr>
                                                <tr>
                                                    <td>8005693381</td>
                                                    <td>7GWA358</td>
                                                    <td>1</td>
                                                    <td>1</td>
                                                    <td>207</td>
                                                    <td>120 x 80 x 107</td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                </div>
                            </article>
                        </div>
                    </div>
                </div>
            </section>
            <!-- Partner -->
            <section class="section-partner">
                <div class="container">
                    <div class="item-partner">
                        <a href=""><img src="images/partner.png" alt="" /></a>
                        <a href=""><img src="images/partner.png" alt="" /></a>
                        <a href=""><img src="images/partner.png" alt="" /></a>
                        <a href=""><img src="images/partner.png" alt="" /></a>
                        <a href=""><img src="images/partner.png" alt="" /></a>
                        <a href=""><img src="images/partner.png" alt="" /></a>
                        <a href=""><img src="images/partner.png" alt="" /></a>
                    </div>
                </div>
            </section>
        </div>
        <!-- Footer -->
        <?php require_once('block/footer.php'); ?>
    </div>
    <?php require_once('block/script.php'); ?>
</body>

</html>
